...
export const Task = ({ task, taskId, onDelete, isDeleting }: Props) => {
  return (
    <>
      <Stack direction={'row'} justifyContent={'space-between'} alignItems={'center'}>
        ...
        <Box sx={{ mb: 5 }}>
          <Stack direction={'row'} spacing={2}>
            <Button variant="contained" href={`/tasks/${taskId}/edit`}>
              編集
            </Button>
            <Button variant="contained" onClick={() => onDelete(taskId)} disabled={isDeleting}>
              削除
            </Button>
          </Stack>
        </Box>
      ...
      <Paper sx={{ padding: 4 }}>
        <Stack spacing={4}>
          ...
          <StackContents spacing={1}>
            ...
            <Typography>{new Date(task.due_at ?? '').toLocaleString()}</Typography>
          </StackContents>
          <StackContents spacing={1}>
            <Typography variant="h6" fontWeight="bold">
              タグ
            ...
            <ChipsArray
              chipsdata={task.tags.map((tag, index) => ({ key: index, label: tag.name }))}
              deletable={false}
            />
          </StackContents>
          <StackContents spacing={1}>
